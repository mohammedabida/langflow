...
# @router.post("/run_tokken/{flow_id_or_name}", response_model_exclude_none=True)  # noqa: RUF100, FAST003
# async def simplified_run_flow(
#     *,
#     background_tasks: BackgroundTasks,
#     flow: Annotated[FlowRead | None, Depends(get_flow_by_id_or_endpoint_name)],
#     input_request: SimplifiedAPIRequest | None = None,
#     stream: bool = False,
#     current_user: Annotated[UserRead, Depends(CurrentActiveUser)],
# ):
#     """Executes a specified flow by ID with support for streaming and telemetry.
...
#         raise HTTPException(status_code=403, detail="You do not have permission to run this flow.")
#     start_time = time.perf_counter()
